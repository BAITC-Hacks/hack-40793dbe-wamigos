...
public class MeetingResultValidator {
    ...
    public void validate(MeetingResultResponse result) {
        ...
        if (result.summary() == null) {
            invalid("Результат анализа не содержит итогов.");
        }
        requireLists(result);

        Set<String> speakerIds = validateSpeakers(result.speakers());
        Set<String> segmentIds = validateSegments(result.segments(), speakerIds);
        validateTasks(result.tasks(), segmentIds);
        validateProblems(result.problems(), segmentIds);
    }

    private Set<String> validateSpeakers(List<SpeakerResponse> speakers) {
        ...
        for (SpeakerResponse speaker : speakers) {
            ...
            requireUnique(speaker.id(), speakerIds);
        }
        return speakerIds;
    }
    ...
    private Set<String> validateSegments(
        ...
        for (SegmentResponse segment : segments) {
            ...
            requireUnique(segment.id(), segmentIds);
            ...
            if (segment.text() == null) {
                invalid("Текст реплики обязателен.");
            }
    ...
    private void validateTasks(List<TaskResponse> tasks, Set<String> segmentIds) {
        Set<String> taskIds = new HashSet<>();
        for (TaskResponse task : tasks) {
            requireNonBlank(task.id(), "Поручение должно иметь идентификатор.");
            requireUnique(task.id(), taskIds);
    ...
    private void validateProblems(
        ...
        for (ProblemResponse problem : problems) {
            ...
            requireUnique(problem.id(), problemIds);
    ...
    private void requireUnique(String id, Set<String> ids) {
        if (!ids.add(id)) {
