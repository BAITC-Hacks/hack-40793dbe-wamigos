...
const result: MeetingResult = {
  ...
  summary: "Обсудили подготовку отчёта и проблему с подключением к Wi-Fi.",
  speakers: [
    { id: "s1", name: "Арнур" },
    { id: "s2", name: "Максат" },
    { id: "s3", name: "Влад" },
  ...
  segments: [
    ...
    },
    {
      ...
      startMs: 28_000,
      endMs: 38_000,
      text: "Не получается подключиться к Wi-Fi.",
      tags: ["PROBLEM"],
    },
    {
      id: "seg3",
      ...
      startMs: 65_000,
      endMs: 75_000,
      text: "Арнур, подготовь отчёт до 15 октября.",
      tags: ["TASK"],
    },
  ],
  tasks: [
  ...
  problems: [
    {
      id: "p1",
      text: "У Максата не работает подключение к Wi-Fi.",
      reportedBy: "Максат",
...
export async function getMock(link: MeetingLink): Promise<Meeting> {
  ...
  const status = link.id.startsWith("demo-")
  ...
  return {
    ...
    expiresAt:
      status === "COMPLETED" || status === "FAILED"
        ? new Date(Date.parse(link.createdAt) + 86_400_000).toISOString()
        : null,
    error:
      status === "FAILED" ? { code: "NO_AUDIO", message: "Нет аудио" } : null,
