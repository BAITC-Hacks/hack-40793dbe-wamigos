...
export class ApiError extends Error {
  constructor(public status: number, public code?: string) { super(code ?? String(status)); }
}
...
const codeMessages: Record<string, string> = {
  FILE_TOO_LARGE: statusMessages[413], UNSUPPORTED_MEDIA_TYPE: statusMessages[415],
  UNSUPPORTED_FORMAT: statusMessages[415], MEETING_NOT_FOUND: statusMessages[404],
  MEETING_EXPIRED: statusMessages[404], EXPORT_NOT_READY: statusMessages[409],
  QUEUE_FULL: statusMessages[503], NO_AUDIO: 'В файле не найдена аудиодорожка.',
  ...
  MOCK_EXPORT_UNAVAILABLE: 'В деморежиме экспорт недоступен. Для скачивания подключите Java API.',
};

export function errorMessage(error: unknown): string {
  if (error instanceof ApiError) return (error.code && codeMessages[error.code]) || statusMessages[error.status] || 'Не удалось выполнить запрос. Попробуйте ещё раз.';
  return 'Нет связи с сервером. Проверьте подключение и повторите запрос.';
}

export function processingError(code?: string): string {
  return (code && codeMessages[code]) || 'Не удалось обработать запись. Попробуйте загрузить другой файл.';
}
