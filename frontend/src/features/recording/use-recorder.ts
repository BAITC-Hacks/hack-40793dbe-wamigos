...
export function useRecorder(onReady: (input: UploadInput) => void) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setElapsed(ms);
      if (ms >= config.maxRecordingMs) stop();
    }, 250);
  ...
  async function start() {
    ...
    if (
      ...
      setError(
        "Запись недоступна в этом браузере. Используйте HTTPS или загрузите MP3/MP4.",
      );
      return;
    }
    ...
    if (!format) {
      setError(
        "Браузер не поддерживает запись в подходящем формате. Загрузите MP3 или MP4.",
      );
      return;
    }
    ...
    try {
      ...
      media.getAudioTracks().forEach((track) => {
        track.onended = () => {
          if (alive.current && recorder.current?.state === "recording") {
            setError(
              "Микрофон отключён. Подключите его и начните запись снова.",
            );
            stop(true);
          }
        };
      ...
      current.ondataavailable = (event) => {
        if (event.data.size) {
          ...
          if (bytes > config.maxBytes && current.state === "recording") {
            setError(
              "Достигнут предел размера записи — 500 МБ. Запишите более короткую встречу.",
            );
            stop(true);
          }
        }
      ...
      current.onstop = () => {
        ...
        if (!blob.size) {
          setError("Микрофон не записал аудио. Попробуйте ещё раз.");
          return;
        }
        if (blob.size > config.maxBytes) {
          setError("Запись превышает 500 МБ. Запишите более короткую встречу.");
          return;
        }
        callback.current({
          file: new File([blob], `Запись встречи.${format.extension}`, {
            type: blob.type,
          }),
          source: "MICROPHONE",
      ...
      if (alive.current) {
        ...
        setError(
          name === "NotAllowedError"
            ? "Разрешите доступ к микрофону в настройках браузера и попробуйте ещё раз или загрузите файл."
            : name === "NotFoundError"
              ? "Микрофон не найден. Подключите его или загрузите файл."
              : "Не удалось включить микрофон. Проверьте, что он доступен, и попробуйте ещё раз.",
        );
      }
    } finally {
