...
interface MeetingsContext {
  links: MeetingLink[]; meetings: Record<string, Meeting>; errors: Record<string, string>;
  loaded: boolean; storageAvailable: boolean; missing: Set<string>;
  add: (link: MeetingLink) => void; refresh: (id: string) => Promise<void>;
}
...
export function MeetingsProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback((id: string): Promise<void> => {
    ...
    const promise = (async () => {
      try {
        ...
        store(linksRef.current.map(item => item.id === id ? { ...item, title: meeting.title, expiresAt: meeting.expiresAt } : item));
      } catch (error) {
        if (!mounted.current || controller.signal.aborted) return;
        if (error instanceof ApiError && error.status === 404) {
          setMissing(previous => new Set(previous).add(id));
          store(linksRef.current.filter(item => item.id !== id));
        } else setErrors(previous => ({ ...previous, [id]: errorMessage(error) }));
      } finally { controllers.current.delete(controller); requests.current.delete(id); }
    })();
  ...
  useEffect(() => {
    ...
    return () => { mounted.current = false; controllers.current.forEach(controller => controller.abort()); requests.current.clear(); };
  }, [refresh]);

  const add = useCallback((link: MeetingLink) => {
    store([link, ...linksRef.current.filter(item => item.id !== link.id)].slice(0, config.maxHistory));
  }, [store]);

  return <Context.Provider value={{ links, meetings, errors, missing, loaded, storageAvailable, add, refresh }}>{children}</Context.Provider>;
}
...
export function useMeetingPolling(ids: string[]) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      const active = key.split(',').filter(Boolean).filter(id => {
        const meeting = states.current.meetings[id];
        return !states.current.errors[id] && (!meeting || meeting.status === 'QUEUED' || meeting.status === 'PROCESSING');
      });
